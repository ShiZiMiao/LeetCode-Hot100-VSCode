...
| 📋 题目列表 | 与官网《热题 100》一致的 100 题：实时显示难度与通过状态（通过打勾）、侧栏总进度（n/100）、分组已解数、状态筛选（未做/已解决/尝试过）、分组方式可切换（按分类/按难度，同难度按题号升序）；**搜索题目**（题号/中文名/英文名/分类，Ctrl+Alt+F）、**随机抽题**（全部/未做优先/按难度/随机错题）、**每日一题**（官网 questionOfToday）；列表展示通过率，会员题 🔒 标记；本地收藏 ⭐（右键题目行切换，离线可用） |
...
| ❌ 错题回顾 | 提交失败自动记录（时间/原因/失败次数），置顶展示可一键重开；**间隔复习排期**（1/3/7/14/30 天到期待复习，到期的排最前）；提交通过自动移出 |
| 📝 题目笔记 | 每题一份本地 Markdown 笔记（`leetcode/notes/{题号}_{slug}.md`，模板含思路/复杂度/备注），题目行右键或命令面板「打开题目笔记」；多根工作区下自动定位到当前正在做的工作区 |
| ▶ 快速运行 | 当前文件一键在终端编译执行 |
| ⌨ 快捷键 | 默认绑定：Ctrl+Alt+T 测试、Ctrl+Alt+S 提交、Ctrl+Alt+→ 下一题、Ctrl+Alt+F 搜索（macOS 为 Cmd+Alt） |
