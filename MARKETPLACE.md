...
| 📋 题目列表 | 与官网《热题 100》一致的 100 题：实时显示难度与通过状态（通过打勾）、侧栏总进度（n/100）、分组已解数、状态筛选（未做/已解决/尝试过）、分组方式可切换（按分类/按难度，同难度按题号升序）；**搜索题目**（题号/中文名/英文名/分类，Ctrl+Alt+F）、**随机抽题**（全部/未做优先/按难度/随机错题）、**每日一题**（从 Hot 100 按日期随机抽取、每天固定一题——侧栏节点直接显示当天题目，无需登录，Ctrl+Alt+D）；列表展示通过率，会员题 🔒 标记；本地收藏 ⭐（右键题目行切换，离线可用，侧栏「⭐ 我的收藏」分组入口） |
| 📝 题面渲染 | 富文本题面/题解，每题单页（重复点击自动聚焦），LaTeX 公式、语法高亮、多语言标签页、官方动画演示（可播放/暂停/逐帧）、官方题解头部信息、**相似题目推荐**（点击直达） |
| ✅ 测试与提交 | 一键运行示例测试、提交判题；通过与否都有完整判题详情（输入/输出/预期/错误原因），可一键跳转官方判题页；**提交历史**（最近 20 条，状态/语言/耗时/时间，点击在浏览器打开；可把云端提交的代码**恢复**回本地——换设备登录同账号即可找回之前提交的代码，跨设备同步；打开题目时若本地无代码文件而云端有历史提交，自动询问恢复）；**下一题**（按当前分组顺序前进，Ctrl+Alt+→） |
| 🐞 本地调试 | 生成含示例与期望输出的调试驱动（判题失败用例可一键带入），链表/树等题型自动还原判题数据结构并逐示例比对，Python 断点直接打在题解文件上 |
| 🧪 自定义用例 | 任意输入用例在线判题（每行一个参数值），也可把判题失败的用例快速带入本地调试 |
| ❌ 错题回顾 | 提交失败自动记录（时间/原因/失败次数），置顶展示可一键重开；**间隔复习排期**（1/3/7/14/30 天到期待复习，到期的排最前）；「复习错题」按到期顺序逐题过一遍（打开即计入今日已复习，记录跨会话保存，全部复习完提示；重新提交通过自动移出队列、失败重新排期）；提交通过自动移出 |
| 📝 题目笔记 | 每题一份本地 Markdown 笔记（`leetcode/notes/{题号}_{slug}.md`，模板含思路/复杂度/备注），题目行右键或命令面板「打开题目笔记」；多根工作区下自动定位到当前正在做的工作区 |
| ▶ 快速运行 | 当前文件一键在终端编译执行 |
| ⌨ 快捷键 | 默认绑定：Ctrl+Alt+T 测试、Ctrl+Alt+S 提交、Ctrl+Alt+→ 下一题、Ctrl+Alt+F 搜索、Ctrl+Alt+D 每日一题、Ctrl+Alt+R 随机抽题、Ctrl+Alt+W 复习错题、Ctrl+Alt+N 题目笔记、Ctrl+Alt+L 收藏（macOS 为 Cmd+Alt） |
...
├── views/hot100Provider.ts   # TreeView（按分类/按难度分组、状态/刷题统计/错题复习队列）
└── utils/
    ├── debugUtils.ts         # 各语言调试驱动生成
    ├── languageUtils.ts      # 语言/扩展名映射与选择
    ├── difficultyGroups.ts   # 按难度分组/排序
    ├── studyStats.ts         # 统计分布/连续打卡
    ├── questionOrder.ts      # 分组视图顺序（下一题）
    └── wrongQueue.ts         # 错题队列/失败次数/复习排期
```
